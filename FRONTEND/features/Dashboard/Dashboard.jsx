...
export default function Dashboard() {
    ...
    const fetchTodaysTasks = async () => {
        const ottData = await apiFetch('ott/due-today', 'GET', token)
        const drtData = await apiFetch('drt/due-today', 'GET', token)
    ...
    const data = tasksToday? tasksToday.map((task) => ({
        id: task.id,
        name: task.name,
    ...
    return(
        <ScreenPrimative scroll style={{ marginBottom: 24 }} edges={[]}>
            ...
            <Surface style={{backgroundColor: 'rgba(255,0,255,0.5)', padding: 16 }}>
